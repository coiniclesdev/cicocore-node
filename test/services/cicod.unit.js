...
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/cico.conf')));
var CicoService = proxyquire('../../lib/services/cicod', {
	fs: {
		readFileSync: readFileSync
	}
});
var defaultCicoConf = fs.readFileSync(path.resolve(__dirname, '../data/default.cico.conf'), 'utf8');

describe('Cico Service', function() {
	...
	var baseConfig = {
		node: {
			network: cicocore.Networks.testnet
		},
		spawn: {
			datadir: 'testdir',
			exec: 'testpath'
	...
	describe('@constructor', function() {
		it('will create an instance', function() {
			var cicod = new CicoService(baseConfig);
			should.exist(cicod);
		});
		it('will create an instance without `new`', function() {
			var cicod = CicoService(baseConfig);
			should.exist(cicod);
		});
		it('will init caches', function() {
			var cicod = new CicoService(baseConfig);

			should.exist(cicod.utxosCache);
			should.exist(cicod.txidsCache);
			should.exist(cicod.balanceCache);
			should.exist(cicod.summaryCache);
			should.exist(cicod.transactionDetailedCache);
			should.exist(cicod.accountInfo);

			should.exist(cicod.transactionCache);
			should.exist(cicod.rawTransactionCache);
			should.exist(cicod.rawJsonTransactionCache);
			should.exist(cicod.blockCache);
			should.exist(cicod.blockJsonCache);
			should.exist(cicod.blockSubsidyCache);
			should.exist(cicod.rawBlockCache);
			should.exist(cicod.blockHeaderCache);
			should.exist(cicod.zmqKnownTransactions);
			should.exist(cicod.zmqKnownBlocks);
			should.exist(cicod.lastTip);
			should.exist(cicod.lastTipTimeout);
			should.equal(cicod.dgpInfoCache, null, 'should be null');
			should.equal(cicod.miningInfoCache, null, 'should be null');
			should.equal(cicod.stakingInfoCache, null, 'should be null');

			// limits
			should.equal(cicod.maxTxids, 1000);
			should.equal(cicod.maxTransactionHistory, 50);
			should.equal(cicod.maxAddressesQuery, 10000);
			should.equal(cicod.shutdownTimeout, 15000);

			// spawn restart setting
			should.equal(cicod.spawnRestartTime, 5000);
			should.equal(cicod.spawnStopTime, 10000);

			// try all interval
			should.equal(cicod.tryAllInterval, 1000);
			should.equal(cicod.startRetryInterval, 5000);

			// rpc limits
			should.equal(cicod.transactionConcurrency, 5);

			// sync progress level when zmq subscribes to events
			should.equal(cicod.zmqSubscribeProgress, 0.9999);
		});
		it('will init clients', function() {
			var cicod = new CicoService(baseConfig);
			cicod.nodes.should.deep.equal([]);
			cicod.nodesIndex.should.equal(0);
			cicod.nodes.push({ client: sinon.stub() });
			should.exist(cicod.client);
		});
		it('will set subscriptions', function() {
			var cicod = new CicoService(baseConfig);
			cicod.subscriptions.should.deep.equal({
				address: {},
				rawtransaction: [],
				hashblock: [],
				balance: {},
	...
	describe('#_initDefaults', function() {
		it('will set transaction concurrency', function() {
			var cicod = new CicoService(baseConfig);
			cicod._initDefaults({ transactionConcurrency: 10 });
			cicod.transactionConcurrency.should.equal(10);
			cicod._initDefaults({});
			cicod.transactionConcurrency.should.equal(5);
		});
	});

	describe('@dependencies', function() {
		it('will have no dependencies', function() {
			CicoService.dependencies.should.deep.equal([]);
		});
	});
	...
	describe('#getAPIMethods', function() {
		it('will return spec', function() {
			var cicod = new CicoService(baseConfig);
			var methods = cicod.getAPIMethods();
	...
	describe('#getPublishEvents', function() {
		it('will return spec', function() {
			var cicod = new CicoService(baseConfig);
			var events = cicod.getPublishEvents();
			should.exist(events);
			events.length.should.equal(4);

			events[0].name.should.equal('cicod/rawtransaction');
			events[0].scope.should.equal(cicod);
			events[0].subscribe.should.be.a('function');
			events[0].unsubscribe.should.be.a('function');

			events[1].name.should.equal('cicod/hashblock');
			events[1].scope.should.equal(cicod);
			events[1].subscribe.should.be.a('function');
			events[1].unsubscribe.should.be.a('function');

			events[2].name.should.equal('cicod/addresstxid');
			events[2].scope.should.equal(cicod);
			events[2].subscribe.should.be.a('function');
			events[2].unsubscribe.should.be.a('function');

			events[3].name.should.equal('cicod/addressbalance');
			events[3].scope.should.equal(cicod);
		...
		it('will call subscribe/unsubscribe with correct args', function() {
			var cicod = new CicoService(baseConfig);
			cicod.subscribe = sinon.stub();
			cicod.unsubscribe = sinon.stub();
			var events = cicod.getPublishEvents();

			events[0].subscribe('test');
			cicod.subscribe.args[0][0].should.equal('rawtransaction');
			cicod.subscribe.args[0][1].should.equal('test');

			events[0].unsubscribe('test');
			cicod.unsubscribe.args[0][0].should.equal('rawtransaction');
			cicod.unsubscribe.args[0][1].should.equal('test');

			events[1].subscribe('test');
			cicod.subscribe.args[1][0].should.equal('hashblock');
			cicod.subscribe.args[1][1].should.equal('test');

			events[1].unsubscribe('test');
			cicod.unsubscribe.args[1][0].should.equal('hashblock');
			cicod.unsubscribe.args[1][1].should.equal('test');
		});
	});
	...
	describe('#subscribe', function() {
		...
		it('will push to subscriptions', function() {
			var cicod = new CicoService(baseConfig);
			var emitter = {};
			cicod.subscribe('hashblock', emitter);
			cicod.subscriptions.hashblock[0].should.equal(emitter);

			var emitter2 = {};
			cicod.subscribe('rawtransaction', emitter2);
			cicod.subscriptions.rawtransaction[0].should.equal(emitter2);
		});
	});
	...
	describe('#unsubscribe', function() {
		...
		it('will remove item from subscriptions', function() {
			...
			var emitter4 = {};
			var emitter5 = {};
			cicod.subscribe('hashblock', emitter1);
			cicod.subscribe('hashblock', emitter2);
			cicod.subscribe('hashblock', emitter3);
			cicod.subscribe('hashblock', emitter4);
			cicod.subscribe('hashblock', emitter5);
			cicod.subscriptions.hashblock.length.should.equal(5);

			cicod.unsubscribe('hashblock', emitter3);
			cicod.subscriptions.hashblock.length.should.equal(4);
			cicod.subscriptions.hashblock[0].should.equal(emitter1);
			cicod.subscriptions.hashblock[1].should.equal(emitter2);
			cicod.subscriptions.hashblock[2].should.equal(emitter4);
			cicod.subscriptions.hashblock[3].should.equal(emitter5);
		});
		it('will not remove item an already unsubscribed item', function() {
			var cicod = new CicoService(baseConfig);
			var emitter1 = {};
			var emitter3 = {};
			cicod.subscriptions.hashblock = [emitter1];
			cicod.unsubscribe('hashblock', emitter3);
			cicod.subscriptions.hashblock.length.should.equal(1);
			cicod.subscriptions.hashblock[0].should.equal(emitter1);
		});
	});
	...
	describe('#subscribeAddress', function() {
		...
		it('will not an invalid address', function() {
			...
			cicod.subscribeAddress(emitter, ['invalidaddress']);
			should.not.exist(cicod.subscriptions.address['invalidaddress']);
		});
		it('will add a valid address', function() {
			var cicod = new CicoService(baseConfig);
			var emitter = new EventEmitter();
			cicod.subscribeAddress(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will handle multiple address subscribers', function() {
			var cicod = new CicoService(baseConfig);
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			cicod.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			cicod.subscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will not add the same emitter twice', function() {
			...
			cicod.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddress', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			cicod.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			cicod.unsubscribeAddress(emitter1);
			cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			cicod.unsubscribeAddress(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			cicod.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
		});
		it('will remove empty addresses', function() {
			...
			cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			cicod.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			cicod.unsubscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will unsubscribe emitter for all addresses', function() {
			...
			var emitter2 = new EventEmitter();
			cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			cicod.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(cicod, 'unsubscribeAddressAll');
			cicod.unsubscribeAddress(emitter1);
			cicod.unsubscribeAddressAll.callCount.should.equal(1);
			cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			cicod.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddressAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			cicod.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			cicod.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			cicod.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			cicod.unsubscribeAddressAll(emitter1);
			cicod.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			cicod.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			cicod.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(cicod.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
		});
	});
	...
	describe('#_getDefaultConfig', function() {
		it('will generate config file from defaults', function() {
			var cicod = new CicoService(baseConfig);
			var config = cicod._getDefaultConfig();
			config.should.equal(defaultCicoConf);
		});
	});
	...
	describe('#_loadSpawnConfiguration', function() {
		...
		it('will parse a cico.conf file', function() {
			var TestCico = proxyquire('../../lib/services/cicod', {
				fs: {
					readFileSync: readFileSync,
			...
			var cicod = new TestCico(baseConfig);
			cicod.options.spawn.datadir = '/tmp/.cico';

			sinon.spy(cicod, '_expandRelativeDatadir');
			sinon.spy(cicod, '_checkConfigIndexes');
			sinon.spy(cicod, '_getDefaultConf')
			sinon.spy(cicod, '_getNetworkConfigPath');
			sinon.spy(cicod, '_parseBitcoinConf');

			var node = {};
			cicod._loadSpawnConfiguration(node);

			cicod._expandRelativeDatadir.callCount.should.equal(1);
			cicod._checkConfigIndexes.callCount.should.equal(1);
			cicod._getDefaultConf.callCount.should.equal(1);
			cicod._getNetworkConfigPath.callCount.should.equal(1);
			cicod._parseBitcoinConf.callCount.should.equal(2);

			should.exist(cicod.spawn.config);
			cicod.spawn.config.should.deep.equal({
				addressindex: 1,
				checkblocks: 144,
				dbcache: 8192,
		...
		it('will expand relative datadir to absolute path', function() {
			...
			var config = {
				node: {
					network: cicocore.Networks.testnet,
					configPath: '/tmp/.cicocore/cicocore-node.json'
				},
				spawn: {
					datadir: './data',
					exec: 'testpath'
			...
			cicod.options.spawn.datadir = './data';
			var node = {};
			cicod._loadSpawnConfiguration(node);
			...
			cicod.options.spawn.datadir.should.equal('/tmp/.cicocore/data');
		});
		it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
			var TestCico = proxyquire('../../lib/services/cicod', {
				fs: {
					readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badcico.conf')),
					existsSync: sinon.stub().returns(true),
				},
				mkdirp: {
					sync: sinon.stub()
			...
			var cicod = new TestCico(baseConfig);
			(function() {
				cicod._loadSpawnConfiguration({ datadir: './test' });
			}).should.throw(cicocore.errors.InvalidState);
		});
	...
	describe('#_checkConfigIndexes', function() {
		...
		it('should warn the user if txindex isn\'t set to 1 in the cico.conf file', function() {
			var cicod = new CicoService(baseConfig);
			var config = {
				txindex: 0,
				addressindex: 1,
			...
			var node = {};
			(function() {
				cicod._checkConfigIndexes(config, node);
			}).should.throw('"txindex" option');
		});
		it('should warn the user if addressindex isn\'t set to 1 in the cico.conf file', function() {
			var cicod = new CicoService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 0,
		...
		it('should warn the user if spentindex isn\'t set to 1 in the cico.conf file', function() {
			var cicod = new CicoService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if server isn\'t set to 1 in the cico.conf file', function() {
			var cicod = new CicoService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if reindex is set to 1 in the cico.conf file', function() {
			var cicod = new CicoService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
	...
	describe('#_resetCaches', function() {
		it('will reset LRU caches', function() {
			...
			for (var i = 0; i < 10; i++) {
				...
				cicod.transactionDetailedCache.set(keys[i], {});
				cicod.utxosCache.set(keys[i], {});
				cicod.accountInfo.set(keys[i], {});
				cicod.txidsCache.set(keys[i], {});
				cicod.balanceCache.set(keys[i], {});
				cicod.summaryCache.set(keys[i], {});
				cicod.blockOverviewCache.set(keys[i], {});
				cicod.blockJsonCache.set(keys[i], {});
			}
			cicod._resetCaches();
			should.equal(cicod.transactionDetailedCache.get(keys[0]), undefined);
			should.equal(cicod.utxosCache.get(keys[0]), undefined);
			should.equal(cicod.accountInfo.get(keys[0]), undefined);
			should.equal(cicod.txidsCache.get(keys[0]), undefined);
			should.equal(cicod.balanceCache.get(keys[0]), undefined);
			should.equal(cicod.summaryCache.get(keys[0]), undefined);
			should.equal(cicod.blockOverviewCache.get(keys[0]), undefined);
			should.equal(cicod.blockJsonCache.get(keys[0]), undefined);
	...
	describe('#_tryAllClients', function() {
		it('will retry for each node client', function(done) {
			...
			cicod._tryAllClients(function(client, next) {
				client.getInfo(next);
			}, function(err) {
				if (err) {
					return done(err);
				}
				cicod.nodes[0].client.getInfo.callCount.should.equal(1);
				cicod.nodes[1].client.getInfo.callCount.should.equal(1);
				cicod.nodes[2].client.getInfo.callCount.should.equal(1);
				done();
			});
		});
		it('will start using the current node index (round-robin)', function(done) {
			...
			cicod._tryAllClients(function(client, next) {
				...
				cicod.nodes[1].client.getInfo.callCount.should.equal(1);
				cicod.nodes[2].client.getInfo.callCount.should.equal(1);
				cicod.nodesIndex.should.equal(2);
				done();
			});
		});
	...
	describe('#_wrapRPCError', function() {
		it('will convert cicod-rpc error object into JavaScript error', function() {
			var cicod = new CicoService(baseConfig);
			var error = cicod._wrapRPCError({ message: 'Test error', code: -1 });
	...
	describe('#_initChain', function() {
		...
		it('will set height and genesis buffer', function(done) {
			...
			cicod.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
			cicod.nodes.push({
				client: {
			...
			cicod._initChain(function() {
				log.info.callCount.should.equal(1);
				cicod.getRawBlock.callCount.should.equal(1);
				cicod.getRawBlock.args[0][0].should.equal('genesishash');
				cicod.height.should.equal(5000);
				cicod.genesisBuffer.should.equal(genesisBuffer);
				done();
			});
		});
		...
		it('it will handle error from getRawBlock', function(done) {
			...
			cicod.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
			cicod._initChain(function(err) {
				err.should.be.instanceOf(Error);
				getBestBlockHash.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
	...
	describe('#_getDefaultConf', function() {
		afterEach(function() {
			cicocore.Networks.disableRegtest();
			baseConfig.node.network = cicocore.Networks.testnet;
		});
		it('will get default rpc port for livenet', function() {
			var config = {
				node: {
					network: cicocore.Networks.livenet
				},
				spawn: {
					datadir: 'testdir',
					exec: 'testpath'
			...
			var cicod = new CicoService(config);
			cicod._getDefaultConf().rpcport.should.equal(8332);
		});
		it('will get default rpc port for testnet', function() {
			...
			var cicod = new CicoService(config);
			cicod._getDefaultConf().rpcport.should.equal(18332);
		});
	...
	describe('#_getNetworkConfigPath', function() {
		...
		it('will get default config path for livenet', function() {
			...
			var cicod = new CicoService(config);
			should.equal(cicod._getNetworkConfigPath(), undefined);
		});
		it('will get default rpc port for testnet', function() {
			var config = {
				node: {
			...
			var cicod = new CicoService(config);
			cicod._getNetworkConfigPath().should.equal('testnet3/cico.conf');
		});
		it('will get default rpc port for regtest', function() {
			cicocore.Networks.enableRegtest();
			var config = {
			...
			var cicod = new CicoService(config);
			cicod._getNetworkConfigPath().should.equal('regtest/cico.conf');
		});
	});
	...
	describe('#_getNetworkOption', function() {
		...
		it('return --testnet for testnet', function() {
			var cicod = new CicoService(baseConfig);
			cicod.node.network = cicocore.Networks.testnet;
			cicod._getNetworkOption().should.equal('--testnet');
		});
		it('return --regtest for testnet', function() {
			var cicod = new CicoService(baseConfig);
			cicod.node.network = cicocore.Networks.testnet;
			cicocore.Networks.enableRegtest();
			cicod._getNetworkOption().should.equal('--regtest');
		});
		it('return undefined for livenet', function() {
			var cicod = new CicoService(baseConfig);
			cicod.node.network = cicocore.Networks.livenet;
			cicocore.Networks.enableRegtest();
			should.equal(cicod._getNetworkOption(), undefined);
		});
	});
	...
	describe('#_zmqBlockHandler', function() {
		it('will emit block', function(done) {
			...
			cicod._rapidProtectedUpdateTip = sinon.stub();
			cicod.on('block', function(block) {
				block.should.equal(message);
				done();
			});
			cicod._zmqBlockHandler(node, message);
			cicod._rapidProtectedUpdateTip.callCount.should.equal(1);
		});
		it('will not emit same block twice', function(done) {
			...
			var message = new Buffer('76843f1dfe455347d897e757ff5ff51a16a0f62b5d02a659c5680392de3a2b89', 'hex');
			cicod._rapidProtectedUpdateTip = sinon.stub();
			sinon.spy(cicod, 'emit');
			cicod.on('block', function(block) {
				block.should.equal(message);
				cicod.emit.callCount.should.equal(1);
				cicod._rapidProtectedUpdateTip.callCount.should.equal(1);
				done();
			});
			cicod._zmqBlockHandler(node, message);
		...
		it('will call function to update tip', function() {
			...
			cicod._zmqBlockHandler(node, message);
			cicod._rapidProtectedUpdateTip.callCount.should.equal(1);
			cicod._rapidProtectedUpdateTip.args[0][0].should.equal(node);
			cicod._rapidProtectedUpdateTip.args[0][1].should.equal(message);
		});
		it('will emit to subscribers', function(done) {
			...
			sinon.spy(cicod, 'emit');
			var emitter = new EventEmitter();
			cicod.subscriptions.hashblock.push(emitter);
			emitter.on('cicod/hashblock', function(blockHash) {
				cicod._rapidProtectedUpdateTip.callCount.should.equal(1);
				cicod.emit.callCount.should.equal(1);
	...
	describe('#_rapidProtectedUpdateTip', function() {
		it('will limit tip updates with rapid calls', function(done) {
			...
			var count = 0;
			function repeat() {
				cicod._rapidProtectedUpdateTip(node, message);
				count++;
				if (count < 50) {
					repeat();
				}
	...
	describe('#_updateTip', function() {
		...
		it('log and emit rpc error from get block', function(done) {
			var cicod = new CicoService(baseConfig);
			cicod.syncPercentage = sinon.stub();
			sinon.spy(cicod, '_resetCaches');
			cicod.on('error', function(err) {
				err.code.should.equal(-1);
				err.message.should.equal('Test error');
				node.client.getBlock.callCount.should.equal(1);
				cicod._resetCaches.callCount.should.equal(1);
			...
			cicod._updateTip(node, message);
		});
		it('emit synced if percentage is 100', function(done) {
			var cicod = new CicoService(baseConfig);
			cicod.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
			cicod.on('synced', function() {
				cicod.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
				done();
			});
			var node = {
		...
		it('NOT emit synced if percentage is less than 100', function(done) {
			var cicod = new CicoService(baseConfig);
			cicod.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
			cicod.on('synced', function() {
				cicod.syncPercentage.callCount.should.equal(0);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('log and emit error from syncPercentage', function(done) {
			var cicod = new CicoService(baseConfig);
			cicod.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			cicod.on('error', function(err) {
				log.error.callCount.should.equal(1);
				cicod.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('reset caches and set height', function(done) {
			var cicod = new CicoService(baseConfig);
			cicod.syncPercentage = sinon.stub();
			cicod._resetCaches = sinon.stub();
			cicod.on('tip', function(height) {
				cicod._resetCaches.callCount.should.equal(1);
				height.should.equal(10);
				cicod.height.should.equal(10);
				done();
			});
			var node = {
				client: {
	...
	describe('#_getAddressesFromTransaction', function() {
		it('will get results using cicocore.Transaction', function() {
			var cicod = new CicoService(baseConfig);
			var wif = 'cVTiUNavxbCzuoaQboDcgEhKYHhgXRaeQ8ZRoYkVXE9mxT1TcSrc';
			var privkey = cicocore.PrivateKey.fromWIF(wif);
			var inputAddress = privkey.toAddress(cicocore.Networks.testnet);
			var outputAddress = cicocore.Address('qMDYc9oXQK19dmLSCQNs5okRt8z4XeMr1X');
			var tx = cicocore.Transaction();
			tx.from({
				txid: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				outputIndex: 0,
				script: cicocore.Script(inputAddress),
				address: inputAddress.toString(),
				satoshis: 5000000000
			});
			tx.to(outputAddress, 5000000000);
			tx.sign(privkey);
			var addresses = cicod._getAddressesFromTransaction(tx);
		...
		it('will handle non-standard script types', function() {
			...
			tx.addOutput(cicocore.Transaction.Output({
				script: cicocore.Script('OP_TRUE'),
				satoshis: 5000000000
			}));
			var addresses = cicod._getAddressesFromTransaction(tx);
		...
		it('will return unique values', function() {
			var cicod = new CicoService(baseConfig);
			var tx = cicocore.Transaction();
			var address = cicocore.Address('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX');
			tx.addOutput(cicocore.Transaction.Output({
				script: cicocore.Script(address),
				satoshis: 5000000000
			}));
			tx.addOutput(cicocore.Transaction.Output({
				script: cicocore.Script(address),
	...
	describe('#_notifyAddressTxidSubscribers', function() {
		it('will emit event if matching addresses', function(done) {
			...
			cicod._getAddressesFromTransaction = sinon.stub().returns([address]);
			var emitter = new EventEmitter();
			cicod.subscriptions.address[address] = [emitter];
			var txid = 'a9f5d9dc39bfff65c12d8a7a8263f57cc187d4c6cc62815d65186726bbc5ecb5';
			var transaction = {};
			emitter.on('cicod/addresstxid', function(data) {
			...
			cicod._notifyAddressTxidSubscribers(txid, transaction);
		});
	...
	describe('#_zmqTransactionHandler', function() {
		it('will emit to subscribers', function(done) {
			...
			var emitter = new EventEmitter();
			cicod.subscriptions.rawtransaction.push(emitter);
			emitter.on('cicod/rawtransaction', function(hex) {
				hex.should.be.a('string');
				hex.should.equal(expectedBuffer.toString('hex'));
				done();
			});
			var node = {};
			cicod._zmqTransactionHandler(node, expectedBuffer);
		});
		it('will NOT emit to subscribers more than once for the same tx', function(done) {
			...
			sinon.spy(cicod, 'emit');
			cicod.subscriptions.rawtransaction.push(emitter);
			emitter.on('cicod/rawtransaction', function() {
				done();
			});
			var node = {};
			cicod._zmqTransactionHandler(node, expectedBuffer);
	...
	describe('#_checkSyncedAndSubscribeZmqEvents', function() {
		...
		it('log errors, update tip and subscribe to zmq events', function(done) {
			var cicod = new CicoService(baseConfig);
			cicod._updateTip = sinon.stub();
			cicod._subscribeZmqEvents = sinon.stub();
			var blockEvents = 0;
			cicod.on('block', function() {
				blockEvents++;
			});
			var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
			...
			cicod._checkSyncedAndSubscribeZmqEvents(node);
			setTimeout(function() {
				...
				cicod._updateTip.callCount.should.equal(11);
				cicod._subscribeZmqEvents.callCount.should.equal(1);
				done();
			}, 200);
		});
	...
	describe('#_subscribeZmqEvents', function() {
		it('will call subscribe on zmq socket', function() {
			...
			cicod._subscribeZmqEvents(node);
		...
		it('will call relevant handler for rawtx topics', function(done) {
			var cicod = new CicoService(baseConfig);
			cicod._zmqTransactionHandler = sinon.stub();
			var node = {
			...
			node.zmqSubSocket.on('message', function() {
				cicod._zmqTransactionHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will call relevant handler for hashblock topics', function(done) {
			var cicod = new CicoService(baseConfig);
			cicod._zmqBlockHandler = sinon.stub();
			var node = {
				zmqSubSocket: new EventEmitter()
			};
			node.zmqSubSocket.subscribe = sinon.stub();
			cicod._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				cicod._zmqBlockHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will ignore unknown topic types', function(done) {
			...
			cicod._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				cicod._zmqBlockHandler.callCount.should.equal(0);
				cicod._zmqTransactionHandler.callCount.should.equal(0);
				done();
			});
	...
	describe('#_initZmqSubSocket', function() {
		it('will setup zmq socket', function() {
			...
			var cicod = new CicoService(baseConfig);
			var node = {};
			cicod._initZmqSubSocket(node, 'url');
	...
	describe('#_checkReindex', function() {
		...
		it('give error from client getblockchaininfo', function(done) {
			...
			cicod._checkReindex(node, function(err) {
				should.exist(err);
		...
		it('will wait until sync is 100 percent', function(done) {
			...
			cicod._checkReindex(node, function() {
	...
	describe('#_loadTipFromNode', function() {
		...
		it('will give rpc error from client getbestblockhash', function(done) {
			...
			cicod._loadTipFromNode(node, function(err) {
				err.should.be.instanceof(Error);
	...
	describe('#_stopSpawnedProcess', function() {
		...
		it('it will kill process and resume', function(done) {
			...
			var TestCicoService = proxyquire('../../lib/services/cicod', {
				fs: {
					readFile: readFile
				}
			});
			var cicod = new TestCicoService(baseConfig);
			cicod.spawnStopTime = 1;
			cicod._process = {};
			cicod._process.kill = sinon.stub();
			cicod._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
				cicod._process.kill.callCount.should.equal(1);
				log.warn.callCount.should.equal(1);
				done();
			});
		});
		it('it will attempt to kill process and resume', function(done) {
			...
			error2.code = 'ESRCH';
			cicod._process.kill = sinon.stub().throws(error2);
			cicod._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
	...
	describe('#_spawnChildProcess', function() {
		...
		it('will give error from spawn config', function(done) {
			var cicod = new CicoService(baseConfig);
			cicod._loadSpawnConfiguration = sinon.stub();
			cicod._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
			cicod._spawnChildProcess(function(err) {
				cicod._loadSpawnConfiguration.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give error from stopSpawnedBitcoin', function() {
			var cicod = new CicoService(baseConfig);
			cicod._loadSpawnConfiguration = sinon.stub();
			cicod._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
			cicod._spawnChildProcess(function(err) {
				cicod._loadSpawnConfiguration.callCount.should.equal(1);
				cicod._stopSpawnedBitcoin.callCount.should.equal(1);
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
			});
		});
		it('will exit spawn if shutdown', function() {
			...
			var cicod = new TestCicoService(config);
			cicod.spawn = {};
			cicod._loadSpawnConfiguration = sinon.stub();
			cicod._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
			cicod.node.stopping = true;
			cicod._spawnChildProcess(function(err) {
				err.should.be.instanceOf(Error);
		...
		it('will include network with spawn command and init zmq/rpc on node', function(done) {
			...
			cicod._loadSpawnConfiguration = sinon.stub();
			cicod.spawn = {};
			cicod.spawn.exec = 'testexec';
			cicod.spawn.configPath = 'testdir/cico.conf';
			cicod.spawn.datadir = 'testdir';
			cicod.spawn.config = {};
			cicod.spawn.config.rpcport = 20001;
			cicod.spawn.config.rpcuser = 'cico';
			cicod.spawn.config.rpcpassword = 'password';
			cicod.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

			cicod._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			cicod._initZmqSubSocket = sinon.stub();
			cicod._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			cicod._checkReindex = sinon.stub().callsArgWith(1, null);
			cicod._spawnChildProcess(function(err, node) {
				should.not.exist(err);
				spawn.callCount.should.equal(1);
				spawn.args[0][0].should.equal('testexec');
				spawn.args[0][1].should.deep.equal([
					'--conf=testdir/cico.conf',
					'--datadir=testdir',
					'--testnet'
				]);
				...
				cicod._loadTipFromNode.callCount.should.equal(1);
				cicod._initZmqSubSocket.callCount.should.equal(1);
				should.exist(cicod._initZmqSubSocket.args[0][0].client);
				cicod._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
				cicod._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
				should.exist(cicod._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
				should.exist(node);
				should.exist(node.client);
				done();
			});
		});
		it('will respawn cicod spawned process', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestCicoService = proxyquire('../../lib/services/cicod', {
				fs: {
			...
			cicod._loadSpawnConfiguration = sinon.stub();
			cicod.spawn = {};
			cicod.spawn.exec = 'cicod';
			cicod.spawn.datadir = '/tmp/cico';
			cicod.spawn.configPath = '/tmp/cico/cico.conf';
			cicod.spawn.config = {};
			cicod.spawnRestartTime = 1;
			cicod._loadTipFromNode = sinon.stub().callsArg(1);
			cicod._initZmqSubSocket = sinon.stub();
			cicod._checkReindex = sinon.stub().callsArg(1);
			cicod._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			cicod._stopSpawnedBitcoin = sinon.stub().callsArg(0);
			sinon.spy(cicod, '_spawnChildProcess');
			cicod._spawnChildProcess(function(err) {
				if (err) {
					return done(err);
				}
				process.once('exit', function() {
					setTimeout(function() {
						cicod._spawnChildProcess.callCount.should.equal(2);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will emit error during respawn', function(done) {
			...
			cicod._spawnChildProcess(function(err) {
				...
				cicod._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
				cicod.on('error', function(err) {
					err.should.be.instanceOf(Error);
					err.message.should.equal('test');
					done();
		...
		it('will NOT respawn cicod spawned process if shutting down', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestCicoService = proxyquire('../../lib/services/cicod', {
				fs: {
			...
			cicod._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						cicod._spawnChildProcess.callCount.should.equal(1);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will give error after 60 retries', function(done) {
			...
			cicod.spawn.config.rpcpassword = 'password';
			cicod.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
			cicod._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
			cicod._spawnChildProcess(function(err) {
				cicod._loadTipFromNode.callCount.should.equal(60);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give error from check reindex', function(done) {
			...
			cicod._initZmqSubSocket = sinon.stub();
			cicod._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			cicod._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
	...
	describe('#_connectProcess', function() {
		it('will give error if connecting while shutting down', function(done) {
			...
			var cicod = new CicoService(config);
			cicod.node.stopping = true;
			cicod.startRetryInterval = 100;
			cicod._loadTipFromNode = sinon.stub();
			cicod._connectProcess({}, function(err) {
				err.should.be.instanceof(Error);
				err.message.should.match(/Stopping while trying to connect to cicod/);
				cicod._loadTipFromNode.callCount.should.equal(0);
				done();
			});
		});
		it('will give error from loadTipFromNode after 60 retries', function(done) {
			...
			cicod.startRetryInterval = 1;
			var config = {};
			cicod._connectProcess(config, function(err) {
				err.should.be.instanceof(Error);
				cicod._loadTipFromNode.callCount.should.equal(60);
				done();
			});
		...
		it('will init zmq/rpc on node', function(done) {
			...
			cicod._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			var config = {};
			cicod._connectProcess(config, function(err, node) {
				should.not.exist(err);
				cicod._loadTipFromNode.callCount.should.equal(1);
				cicod._initZmqSubSocket.callCount.should.equal(1);
				cicod._loadTipFromNode.callCount.should.equal(1);
	...
	describe('#start', function() {
		...
		it('will give error if "spawn" and "connect" are both not configured', function(done) {
			var cicod = new CicoService(baseConfig);
			cicod.options = {};
			cicod.start(function(err) {
				err.should.be.instanceof(Error);
				err.message.should.be.a('string').and.equal('Cico configuration options "spawn" or "connect" are expected');
				done();
			});
		});
		...
		it('will give error from connectProcess', function(done) {
			var cicod = new CicoService(baseConfig);
			cicod._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
			cicod.options = {
				connect: [
					{}
				]
			...
			cicod.start(function(err) {
				cicod._connectProcess.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will push node from spawnChildProcess', function(done) {
			var cicod = new CicoService(baseConfig);
			var node = {};
			cicod._initChain = sinon.stub().callsArg(0);
			cicod._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
			cicod.options = {
				spawn: {}
			};
			cicod.start(function(err) {
				should.not.exist(err);
				cicod.nodes.length.should.equal(1);
				done();
			});
		});
		it('will push node from connectProcess', function(done) {
			...
			sinon.spy(cicod, '_spawnChildProcess');
			var nodes = [{}];
			cicod._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
			cicod.options = {
				connect: [
					{}
				]
			...
			cicod.start(function(err) {
				should.not.exist(err);
				cicod._initChain.callCount.should.equal(1);
				cicod._connectProcess.callCount.should.equal(1);
				cicod.nodes.length.should.equal(1);
				done();
			});
	...
	describe('#isSynced', function() {
		...
		it('will give "true" if percentage is 100.00', function(done) {
			var cicod = new CicoService(baseConfig);
			cicod.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
			cicod.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
				cicod.syncPercentage.callCount.should.equal(1);
		...
		it('will give "true" if percentage is 99.98', function(done) {
			var cicod = new CicoService(baseConfig);
			cicod.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
			cicod.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 99.49', function(done) {
			var cicod = new CicoService(baseConfig);
			cicod.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
			cicod.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 1', function(done) {
			var cicod = new CicoService(baseConfig);
			cicod.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
			cicod.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
	...
	describe('#syncPercentage', function() {
		it('will give rpc error', function(done) {
			...
			cicod.syncPercentage(function(err) {
				should.exist(err);
				cicod.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
		...
		it('will call client getInfo and give result', function(done) {
			...
			cicod.syncPercentage(function(err, percentage) {
				if (err) {
					return done(err);
				}
				cicod.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
	...
	describe('#_normalizeAddressArg', function() {
		it('will turn single address into array', function() {
			var cicod = new CicoService(baseConfig);
			var args = cicod._normalizeAddressArg('address');
			args.should.deep.equal(['address']);
		});
		it('will keep an array as an array', function() {
			var cicod = new CicoService(baseConfig);
			var args = cicod._normalizeAddressArg(['address one', 'address two']);
	...
	describe('#getAddressBalance', function() {
		it('will give rpc error', function(done) {
			var cicod = new CicoService(baseConfig);
			sinon.spy(cicod, '_normalizeAddressArg');
			cicod.nodes.push({
				client: {
			...
			var options = {};
			cicod.getAddressBalance(address, options, function(err) {
				cicod._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give balance', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			var options = {};
			cicod.getAddressBalance(address, options, function(err, data) {
				if (err) {
					return done(err);
				}
				data.balance.should.equal(10000);
				data.received.should.equal(100000);
				cicod.getAddressBalance(address, options, function(err, data2) {
					if (err) {
						return done(err);
					}
					data2.balance.should.equal(10000);
					data2.received.should.equal(100000);
					cicod._normalizeAddressArg.callCount.should.equal(2);
	...
	describe('#getAddressUnspentOutputs', function() {
		it('will give rpc error', function(done) {
			...
			cicod.getAddressUnspentOutputs(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give results from client getaddressutxos', function(done) {
			...
			var address = 'qMYemSxspKpzB2RbDunJRosQv5PJd3eEWD';
			cicod.getAddressUnspentOutputs(address, options, function(err, utxos) {
				if (err) {
					return done(err);
				}
	...
	describe('#_getBalanceFromMempool', function() {
		it('will sum satoshis', function() {
			...
			var sum = cicod._getBalanceFromMempool(deltas);
	...
	describe('#_getTxidsFromMempool', function() {
		it('will filter to txids', function() {
			...
			var txids = cicod._getTxidsFromMempool(deltas);
	...
	describe('#_getHeightRangeQuery', function() {
		it('will detect range query', function() {
			...
			var rangeQuery = cicod._getHeightRangeQuery(options);
		...
		it('will get range properties', function() {
			...
			var clone = {};
			var rangeQuery = cicod._getHeightRangeQuery(options, clone);
		...
		it('will throw error with invalid range', function() {
			...
			(function() {
				cicod._getHeightRangeQuery(options);
	...
	describe('#getAddressTxids', function() {
		it('will give error from _getHeightRangeQuery', function(done) {
			var cicod = new CicoService(baseConfig);
			cicod._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
			cicod.getAddressTxids('address', {}, function(err) {
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give rpc error from mempool query', function() {
			...
			sinon.spy(cicod, '_getHeightRangeQuery');
			sinon.spy(cicod, '_normalizeAddressArg');
			...
			var options = {};
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			cicod.getAddressTxids(address, options, function(err) {
				should.exist(err);
				cicod._getHeightRangeQuery.callCount.should.equal(1);
				cicod._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
			});
		});
		...
		it('will get txid results', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			cicod.getAddressTxids(address, options, function(err, txids) {
				if (err) {
					return done(err);
				}
				cicod._getHeightRangeQuery.callCount.should.equal(1);
		...
		it('will get txid results from cache', function(done) {
			...
			cicod.getAddressTxids(address, options, function(err, txids) {
				...
				cicod.getAddressTxids(address, options, function(err, txids) {
					...
					cicod._getHeightRangeQuery.callCount.should.equal(2);
					cicod._normalizeAddressArg.callCount.should.equal(2);
					getAddressTxids.callCount.should.equal(1);
					txids.should.deep.equal(expectedTxids);
					done();
		...
		it('will get txid results WITHOUT cache if rangeQuery and exclude mempool', function(done) {
			...
			cicod.getAddressTxids(address, options, function(err, txids) {
				...
				cicod.getAddressTxids(address, options, function(err, txids) {
					...
					cicod._getHeightRangeQuery.callCount.should.equal(4);
					cicod._normalizeAddressArg.callCount.should.equal(2);
		...
		it('will get txid results from cache and live mempool', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			cicod.getAddressTxids(address, { queryMempool: false }, function(err, txids) {
				if (err) {
					return done(err);
				}
				getAddressTxids.callCount.should.equal(1);
				...
				cicod.getAddressTxids(address, { queryMempool: true }, function(err, txids) {
					if (err) {
						return done(err);
					}
					getAddressTxids.callCount.should.equal(1);
					...
					cicod.getAddressTxids(address, { queryMempoolOnly: true }, function(err, txids) {
						if (err) {
							return done(err);
						}
						getAddressTxids.callCount.should.equal(1);
						...
						cicod._getHeightRangeQuery.callCount.should.equal(3);
						cicod._normalizeAddressArg.callCount.should.equal(3);
						done();
					});
				});
			});
	...
	describe('#_getConfirmationDetail', function() {
		...
		it('should get 0 confirmation', function() {
			...
			var cicod = new CicoService(baseConfig);
			cicod.height = 10;
			var confirmations = cicod._getConfirmationsDetail(tx);
	...
	describe('#_getAddressDetailsForInput', function() {
		it('will return if missing an address', function() {
			var cicod = new CicoService(baseConfig);
			var result = {};
			cicod._getAddressDetailsForInput({}, 0, result, []);
		...
		it('will only add address if it matches', function() {
			var cicod = new CicoService(baseConfig);
			var result = {};
			cicod._getAddressDetailsForInput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForOutput', function() {
		it('will return if missing an address', function() {
			var cicod = new CicoService(baseConfig);
			var result = {};
			cicod._getAddressDetailsForOutput({}, 0, result, []);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will only add address if it matches', function() {
	...
	describe('#_getAddressDetailsForTransaction', function() {
		it('will calculate details for the transaction', function(done) {
			...
			sinon.spy(cicod, '_getAddressDetailsForInput');
			sinon.spy(cicod, '_getAddressDetailsForOutput');

			var addresses = ['qey8KJ8xkyLUUqFZZwqngXX9U2q9ef6Tw4'];
			var details = cicod._getAddressDetailsForTransaction(tx, addresses);
			...
			cicod._getAddressDetailsForInput.callCount.should.equal(1);
			cicod._getAddressDetailsForOutput.callCount.should.equal(5);
			done();
		});
	});
	...
	describe('#_getAddressDetailedTransaction', function() {
		it('will get detailed transaction info', function(done) {
			...
			var cicod = new CicoService(baseConfig);
			sinon.spy(cicod, '_getConfirmationsDetail');
			cicod.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
			cicod.height = 300;
			var addresses = {};
			cicod._getAddressDetailsForTransaction = sinon.stub().returns({
				addresses: addresses,
				satoshis: 1000,
			});
			cicod._getAddressDetailedTransaction(txid, {}, function(err, details) {
				if (err) {
					return done(err);
				}
				details.addresses.should.equal(addresses);
				details.satoshis.should.equal(1000);
				cicod.getDetailedTransaction.callCount.should.equal(1);
				cicod._getAddressDetailsForTransaction.callCount.should.equal(1);
				cicod._getConfirmationsDetail.callCount.should.equal(1);
		...
		it('give error from _getDetailedTransaction', function(done) {
			...
			var cicod = new CicoService(baseConfig);
			cicod.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
			cicod._getAddressDetailedTransaction(txid, {}, function(err) {
				err.should.be.instanceof(Error);
				cicod.getDetailedTransaction.callCount.should.equal(1);
				done();
			});
	...
	describe('#_getAddressStrings', function() {
		it('will get address strings from cicocore addresses', function() {
			var addresses = [
				cicocore.Address('qU12Fa5RHM535kSDvywxPjCmbL7gwkQJZ6'),
				cicocore.Address('qcSLSxN1sngCWSrKFZ6UC7ri4hhVSdq9SU'),
			];
			var cicod = new CicoService(baseConfig);
			var strings = cicod._getAddressStrings(addresses);
		...
		it('will give error with unknown', function() {
			...
			var cicod = new CicoService(baseConfig);
			(function() {
				cicod._getAddressStrings(addresses);
	...
	describe('#getAddressHistory', function() {
		...
		it('will give an error if length of addresses is too long', function(done) {
			...
			var cicod = new CicoService(baseConfig);
			sinon.spy(cicod, '_normalizeAddressArg');
			cicod.maxAddressesQuery = 100;
			cicod.getAddressHistory(addresses, {}, function(err) {
				should.exist(err);
				cicod._normalizeAddressArg.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" range that exceeds max size', function(done) {
			var cicod = new CicoService(baseConfig);
			sinon.spy(cicod, '_normalizeAddressArg');
			sinon.spy(cicod, '_getAddressStrings');
			cicod.getAddressHistory(address, { from: 0, to: 51 }, function(err) {
				should.exist(err);
				cicod._normalizeAddressArg.callCount.should.equal(1);
				cicod._getAddressStrings.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" order is reversed', function(done) {
			...
			sinon.spy(cicod, '_normalizeAddressArg');
			sinon.spy(cicod, '_getAddressStrings');
			cicod.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
			cicod.getAddressHistory(address, { from: 51, to: 0 }, function(err) {
				should.exist(err);
				cicod._normalizeAddressArg.callCount.should.equal(1);
				cicod._getAddressStrings.callCount.should.equal(1);
				err.message.match(/^\"from/);
		...
		it('will give error from _getAddressDetailedTransaction', function(done) {
			...
			sinon.spy(cicod, '_normalizeAddressArg');
			sinon.spy(cicod, '_getAddressStrings');
			cicod.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
			cicod._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
			cicod.getAddressHistory(address, {}, function(err) {
				should.exist(err);
				cicod._normalizeAddressArg.callCount.should.equal(1);
				cicod._getAddressStrings.callCount.should.equal(1);
				cicod.getAddressTxids.callCount.should.equal(1);
				cicod._getAddressDetailedTransaction.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
			});
		});
		it('give error from getAddressTxids', function(done) {
			...
			sinon.spy(cicod, '_normalizeAddressArg');
			sinon.spy(cicod, '_getAddressStrings');
			sinon.spy(cicod, 'getAddressTxids');
			cicod.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			cicod.getAddressHistory('address', {}, function(err) {
				should.exist(err);
				cicod._normalizeAddressArg.callCount.should.equal(1);
				cicod._getAddressStrings.callCount.should.equal(1);
		...
		it('will paginate', function(done) {
			...
			sinon.spy(cicod, '_normalizeAddressArg');
			sinon.spy(cicod, '_getAddressStrings');
			sinon.spy(cicod, '_paginateTxids');
			cicod._getAddressDetailedTransaction = function(txid, options, callback) {
				callback(null, txid);
			};
			sinon.spy(cicod, '_getAddressDetailedTransaction');
			var txids = ['one', 'two', 'three', 'four'];
			cicod.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
			cicod.getAddressHistory('address', { from: 1, to: 3 }, function(err, data) {
				if (err) {
					return done(err);
				}
				cicod._normalizeAddressArg.callCount.should.equal(1);
				cicod._getAddressStrings.callCount.should.equal(1);
				cicod.getAddressTxids.callCount.should.equal(1);
				cicod._getAddressDetailedTransaction.callCount.should.equal(2);
				cicod._paginateTxids.callCount.should.equal(1);
	...
	describe('#getAddressSummary', function() {
		...
		it('will handle error from getAddressTxids', function(done) {
			...
			cicod.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			cicod.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
			var address = '';
			var options = {};
			cicod.getAddressSummary(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
		...
		it('will handle error from getAddressBalance', function(done) {
			...
			cicod.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
			cicod.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
			var address = '';
			var options = {};
			cicod.getAddressSummary(address, options, function(err) {
				should.exist(err);
		...
		it('should set all properties', function(done) {
			...
			cicod.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
			cicod.getAddressBalance = sinon.stub().callsArgWith(2, null, {
				received: 30 * 1e8,
				balance: 20 * 1e8
			});
			...
			var options = {};
			cicod.getAddressSummary(address, options, function(err, summary) {
				cicod._paginateTxids.callCount.should.equal(1);
				cicod._normalizeAddressArg.callCount.should.equal(1);
				cicod.getAddressTxids.callCount.should.equal(1);
				cicod.getAddressBalance.callCount.should.equal(1);
				cicod._paginateTxids.args[0][1].should.equal(0);
				cicod._paginateTxids.args[0][2].should.equal(1000);
		...
		it('will get from cache with noTxList', function(done) {
			...
			cicod.getAddressSummary(address, options, function(err, summary) {
				...
				cicod.getAddressSummary(address, options, function(err, summary) {
					checkSummary(summary);
					cicod._paginateTxids.callCount.should.equal(0);
					cicod._normalizeAddressArg.callCount.should.equal(2);
					cicod.getAddressTxids.callCount.should.equal(1);
					cicod.getAddressBalance.callCount.should.equal(1);
					done();
		...
		it('will skip querying the mempool with queryMempool set to false', function(done) {
			...
			cicod.getAddressSummary(address, options, function() {
				getAddressMempool.callCount.should.equal(0);
				cicod._paginateTxids.callCount.should.equal(1);
				cicod._normalizeAddressArg.callCount.should.equal(1);
				cicod.getAddressTxids.callCount.should.equal(1);
		...
		it('will give error from _paginateTxids', function(done) {
			...
			cicod._paginateTxids = sinon.stub().throws(new Error('test'));
			var address = 'qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR';
			var options = {
				queryMempool: false
			};
	...
	describe('#getRawBlock', function() {
		...
		it('will give rpc error from client getblockhash', function(done) {
			...
			cicod.getRawBlock(10, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give rcp error from client getblock', function(done) {
			...
			sinon.spy(cicod, '_tryAllClients');
			cicod.nodes.push({
				client: {
					getBlock: sinon.stub().callsArgWith(2, { code: -1, message: 'Test error' })
				}
			});
			cicod.getRawBlock(blockhash, function(err) {
				should.exist(err);
				cicod._tryAllClients.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		});
		it('will try all nodes for getblock', function(done) {
			...
			cicod.getRawBlock(blockhash, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
		...
		it('will get block by height', function(done) {
			...
			cicod.getRawBlock(0, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
	...
	describe('#getBlock', function() {
		...
		it('will give an rpc error from client getblock', function(done) {
			...
			cicod.getBlock(0, function(err) {
				cicod._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
		...
		it('will getblock as cicocore object from height', function(done) {
			var cicod = new CicoService(baseConfig);
			sinon.spy(cicod, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			cicod.getBlock(0, function(err, block) {
				should.not.exist(err);
				cicod._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				...
				block.should.be.instanceof(cicocore.Block);
				done();
			});
		});
		it('will getblock as cicocore object', function(done) {
			var cicod = new CicoService(baseConfig);
			sinon.spy(cicod, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			cicod.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
				should.not.exist(err);
				cicod._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache', function(done) {
			...
			cicod.getBlock(hash, function(err, block) {
				should.not.exist(err);
				cicod._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache with height (but not height)', function(done) {
			...
			cicod.getBlock(0, function(err, block) {
				...
				cicod.getBlock(0, function(err, block) {
					should.not.exist(err);
					cicod._tryAllClients.callCount.should.equal(3);
	...
	describe('#getBlockHashesByTimestamp', function() {
		it('should give an rpc error', function(done) {
			...
			cicod.getBlockHashesByTimestamp(1517410000, 1517843954, function(err, hashes) {
				should.exist(err);
	...
	describe('#getBlockHeader', function() {
		...
		it('will give error from getBlockHash', function() {
			var cicod = new CicoService(baseConfig);
			sinon.spy(cicod, '_maybeGetBlockHash');
			var getBlockHash = sinon.stub().callsArgWith(1, { code: -1, message: 'Test error' });
			cicod.nodes.push({
				client: {
					getBlockHash: getBlockHash
			...
			cicod.getBlockHeader(10, function(err) {
				cicod._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockheader', function() {
			...
			cicod.getBlockHeader(blockhash, function(err) {
				cicod._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockhash', function() {
			...
			cicod.getBlockHeader(0, function(err) {
				cicod._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('will give result from client getblockheader (from height)', function() {
			...
			cicod.getBlockHeader(81419, function(err, blockHeader) {
				should.not.exist(err);
				cicod._maybeGetBlockHash.callCount.should.equal(1);
		...
		it('will give result from client getblockheader (from hash)', function() {
			...
			cicod.getBlockHeader(blockhash, function(err, blockHeader) {
				should.not.exist(err);
				cicod._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				blockHeader.should.deep.equal(result);
	...
	describe('#_maybeGetBlockHash', function() {
		it('will not get block hash with an address', function(done) {
			...
			cicod._maybeGetBlockHash('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX', function(err, hash) {
				if (err) {
					return done(err);
				}
				cicod._tryAllClients.callCount.should.equal(0);
				getBlockHash.callCount.should.equal(0);
		...
		it('will not get block hash with non zero-nine numeric string', function(done) {
			...
			cicod._maybeGetBlockHash('109a', function(err, hash) {
				if (err) {
					return done(err);
				}
				getBlockHash.callCount.should.equal(0);
		...
		it('will get the block hash if argument is a number', function(done) {
			...
			cicod._maybeGetBlockHash(10, function(err, hash) {
				if (err) {
					return done(err);
				}
	...
	describe('#getBlockOverview', function() {
		...
		it('will handle error from maybeGetBlockHash', function(done) {
			var cicod = new CicoService(baseConfig);
			sinon.spy(cicod, '_maybeGetBlockHash');
			cicod._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
			cicod.getBlockOverview(blockhash, function(err) {
				err.should.be.instanceOf(Error);
				cicod._maybeGetBlockHash.callCount.should.equal(1);
				done();
			});
		...
		it('will give expected result', function(done) {
			...
			cicod.getBlockOverview(blockhash, function(err, blockOverview) {
				if (err) {
					return done(err);
				}
				...
				cicod.getBlockOverview(blockhash, function(err, blockOverview) {
					checkBlock(blockOverview);
					cicod._maybeGetBlockHash.callCount.should.equal(2);
					getBlock.callCount.should.equal(1);
					done();
				});
			});
	...
	describe('#estimateFee', function() {
		it('will give rpc error', function(done) {
			...
			cicod.estimateFee(1, function(err) {
				should.exist(err);
		...
		it('will call client estimateFee and give result', function(done) {
			...
			cicod.estimateFee(1, function(err, feesPerKb) {
				if (err) {
					return done(err);
				}
				estimateFee.callCount.should.equal(1);
	...
	describe('#sendTransaction', function(done) {
		var tx = cicocore.Transaction(txhex);
		it('will give rpc error', function() {
			...
			cicod.sendTransaction(txhex, function(err) {
				should.exist(err);
		...
		it('will send to client and get hash', function() {
			...
			cicod.sendTransaction(txhex, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('will send to client with absurd fees and get hash', function() {
			...
			cicod.sendTransaction(txhex, { allowAbsurdFees: true }, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('missing callback will throw error', function() {
			...
			var transaction = cicocore.Transaction();
			(function() {
				cicod.sendTransaction(transaction);
			}).should.throw(Error);
			sendRawTransaction.callCount.should.equal(1);
		});
	});
	...
	describe('#getRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			cicod.getRawTransaction('txid', function(err) {
				should.exist(err);
				cicod._tryAllClients.callCount.should.equal(1);
		...
		it('will try all nodes', function(done) {
			...
			cicod.getRawTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
	...
	describe('#getTransaction', function() {
		it('will give rpc error', function(done) {
			...
			cicod.getTransaction('txid', function(err) {
				should.exist(err);
				cicod._tryAllClients.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will try all nodes', function(done) {
			...
			cicod.getTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
				...
				getRawTransaction.callCount.should.equal(1);
				cicod._tryAllClients.callCount.should.equal(1);
				tx.should.be.an.instanceof(cicocore.Transaction);
				done();
			});
		});
		it('will get from cache', function(done) {
	...
	describe('#getDetailedTransaction', function() {
		...
		it('will give rpc error', function(done) {
			...
			cicod.getDetailedTransaction(txid, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				cicod._tryAllClients.callCount.should.equal(1);
				done();
		...
		it('should give a transaction with all properties', function(done) {
			...
			cicod.getDetailedTransaction(txid, function(err, tx) {
				if (err) {
					return done(err);
				}
				checkTx(tx);
	...
	describe('#getBestBlockHash', function() {
		it('will give rpc error', function(done) {
			...
			cicod.getBestBlockHash(function(err) {
				should.exist(err);
				getBestBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will call client getInfo and give result', function(done) {
			...
			cicod.getBestBlockHash(function(err, hash) {
				if (err) {
					return done(err);
				}
				getBestBlockHash.callCount.should.equal(1);
	...
	describe('#getSpentInfo', function() {
		it('will give rpc error', function(done) {
			...
			cicod.getSpentInfo({}, function(err) {
				should.exist(err);
		...
		it('will empty object when not found', function(done) {
			...
			cicod.getSpentInfo({}, function(err, info) {
				should.not.exist(err);
				getSpentInfo.callCount.should.equal(1);
	...
	describe('#getInfo', function() {
		...
		it('will call client getInfo and give result', function(done) {
			var cicod = new CicoService(baseConfig);
			cicod.node.getNetworkName = sinon.stub().returns('testnet');
			...
			cicod.getSubsidy = sinon.stub().callsArgWith(1, null, 400000000);
			cicod.nodes.push({
				client: {
					getInfo: getInfo,
				},
			...
			cicod.getInfo(function(err, info) {
				if (err) {
					return done(err);
				}

				cicod.getSubsidy.callCount.should.equal(1);
				cicod.client.getInfo.callCount.should.equal(1);
	...
	describe('#generateBlock', function() {
		it('will give rpc error', function(done) {
			...
			cicod.generateBlock(10, function(err) {
				should.exist(err);
		...
		it('will call client generate and give result', function(done) {
			...
			cicod.generateBlock(10, function(err, hashes) {
				if (err) {
					return done(err);
				}
	...
	describe('#stop', function() {
		...
		it('will exit spawned process', function(done) {
			var cicod = new CicoService(baseConfig);
			cicod.spawn = {};
			cicod.spawn.process = new EventEmitter();
			cicod.spawn.process.kill = sinon.stub();
			cicod.stop(done);
			cicod.spawn.process.kill.callCount.should.equal(1);
			cicod.spawn.process.kill.args[0][0].should.equal('SIGINT');
			cicod.spawn.process.emit('exit', 0);
		});
		it('will give error with non-zero exit status code', function(done) {
			...
			cicod.spawn.process.kill.callCount.should.equal(1);
			cicod.spawn.process.kill.args[0][0].should.equal('SIGINT');
			cicod.spawn.process.emit('exit', 1);
		});
		it('will stop after timeout', function(done) {
			var cicod = new CicoService(baseConfig);
			cicod.shutdownTimeout = 300;
			cicod.spawn = {};
			cicod.spawn.process = new EventEmitter();
			cicod.spawn.process.kill = sinon.stub();
			cicod.stop(function(err) {
	...
	describe('#getAddressesMempoolBalance', function() {
		it('will give rpc error', function(done) {
			...
			cicod.getAddressesMempoolBalance([], {}, function(err, result) {
				should.exist(err);
				cicod._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will calc mempool balance', function(done) {
			...
			var cicod = new CicoService(baseConfig);
			sinon.spy(cicod, '_normalizeAddressArg');
			sinon.spy(cicod, '_getBalanceFromMempool');
			var getAddressMempool = sinon.stub().callsArgWith(1, null, {
				result: deltas
			});
			cicod.nodes.push({
			...
			cicod.getAddressesMempoolBalance([], {}, function(err, result) {
				should.not.exist(err);
				cicod._getBalanceFromMempool.callCount.should.equal(1);
				cicod._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
	...
	describe('#getJsonRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			cicod.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err) {
				should.exist(err);
				cicod._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give transaction', function(done) {
			...
			cicod.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err, response) {
				should.not.exist(err);
				cicod._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
	...
	describe('#getSubsidy', function() {
		it('will give rpc error', function(done) {
			...
			cicod.getSubsidy(0, function(err) {
				should.exist(err);
		...
		it('will give subsidy', function(done) {
			...
			cicod.getSubsidy(0, function(err, result) {
				should.not.exist(err);
				getSubsidy.callCount.should.equal(1);
	...
	describe('#getJsonBlock', function() {
		it('will give rpc error from getBlockHash', function(done) {
			...
			cicod.getJsonBlock(0, function(err) {
				should.exist(err);
				cicod._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give rpc error from getBlock', function(done) {
			...
			cicod.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err) {
				should.exist(err);
				cicod._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give jsonBlock using block height', function(done) {
			...
			cicod.getJsonBlock(80000, function(err, result) {
				should.not.exist(err);
				cicod._tryAllClients.callCount.should.equal(2);
				cicod._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
		...
		it('will give jsonBlock using blockhash', function(done) {
			...
			cicod.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err, result) {
				should.not.exist(err);
				cicod._tryAllClients.callCount.should.equal(1);
				cicod._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
	...
	describe('#subscribeBalance', function() {
		...
		it('will not add an invalid address', function() {
			var cicod = new CicoService(baseConfig);
			var emitter = new EventEmitter();
			cicod.subscribeBalance(emitter, ['invalidaddress']);
			should.not.exist(cicod.subscriptions.balance['invalidaddress']);
			log.info.callCount.should.equal(1);
		});
		it('will add a valid address', function() {
			var cicod = new CicoService(baseConfig);
			var emitter = new EventEmitter();
			cicod.subscribeBalance(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(1);
		...
		it('will handle multiple address subscribers', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			cicod.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			cicod.subscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		...
		it('will not add the same emitter twice', function() {
			...
			cicod.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(2);
		});
	});
	...
	describe('#unsubscribeBalance', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			cicod.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		...
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			cicod.unsubscribeBalance(emitter1);
			cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			cicod.unsubscribeBalance(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			cicod.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
			log.info.callCount.should.equal(1);
		});
		it('will remove empty addresses', function() {
			var cicod = new CicoService(baseConfig);
			...
			cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			cicod.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			cicod.unsubscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(2);
		});
		it('will unsubscribe emitter for all addresses', function() {
			var cicod = new CicoService(baseConfig);
			...
			var emitter2 = new EventEmitter();
			cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			cicod.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(cicod, 'unsubscribeBalanceAll');
			cicod.unsubscribeBalance(emitter1);
			cicod.unsubscribeBalanceAll.callCount.should.equal(1);
			cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			cicod.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#unsubscribeBalanceAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			cicod.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			cicod.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			cicod.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			cicod.unsubscribeBalanceAll(emitter1);
			cicod.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			cicod.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			cicod.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(cicod.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#_notifyBalanceSubscribers', function() {
		...
		it('will notify balance subscribers', function(done) {
			...
			var cicod = new CicoService(baseConfig);
			sinon.spy(cicod, '_notifyBalanceSubscriber');
			cicod._getAddressesFromTransaction = sinon.stub().returns(addresses);
			cicod.getAddressSummary = sinon.stub().callsArgWith(2, null, {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				cicod._notifyBalanceSubscriber.callCount.should.equal(1);
				cicod._getAddressesFromTransaction.callCount.should.equal(1);
				emitter.emit.callCount.should.equal(1);
				done();
			});
			...
			cicod._notifyBalanceSubscribers('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', transaction);
	...
	describe('#_notifyBalanceSubscriber', function() {
		...
		it('will notify balance subscriber', function(done) {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				data.txid.should.equal('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
				emitter.emit.callCount.should.equal(1);
				cicod.getAddressSummary.callCount.should.equal(1);
				done();
			});
			...
			cicod._notifyBalanceSubscriber('qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z', '8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
	...
	describe('#listUnspent', function() {
		it('will give rpc error', function(done) {
			...
			cicod.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err) {
				should.exist(err);
		...
		it('will give array of unspent transaction outputs', function(done) {
			...
			cicod.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err, result) {
				should.not.exist(err);
				listUnspent.callCount.should.equal(1);
	...
	describe('#getNewAddress', function() {
		it('will give rpc error', function(done) {
			...
			cicod.getNewAddress(function(err, result) {
				should.exist(err);
	...
	describe('#callContract', function() {
		it('will give rpc error', function(done) {
			...
			cicod.callContract('f6177bc9812eeb531907621af6641a41133dea9e', 'd7bb99ba', {}, function(err, result) {
				should.exist(err);
	...
	describe('#getAccountInfo', function() {
		it('will give rpc error', function(done) {
			...
			cicod.getAccountInfo('f6177bc9812eeb531907621af6641a41133dea9e', function(err, result) {
				should.exist(err);
	...
	describe('#getTransactionReceipt', function() {
		it('will give rpc error', function(done) {
			...
			cicod.getTransactionReceipt('51b34cbcaf6fe3b687bf3d954f3baaeb377f5d0a3a8cdd29d899c049f6954a49', function(err, result) {
				should.exist(err);
				cicod._tryAllClients.callCount.should.equal(1);
	...
	describe('#getDgpInfo', function() {
		it('will give rpc error', function(done) {
			...
			cicod.getDgpInfo(function(err, result) {
				should.exist(err);
				cicod._tryAllClients.callCount.should.equal(1);
	...
	describe('#getMiningInfo', function() {
		it('will give rpc error', function(done) {
			...
			cicod.getMiningInfo(function(err, result) {
				should.exist(err);
				cicod._tryAllClients.callCount.should.equal(1);
	...
	describe('#getStakingInfo', function() {
		it('will give rpc error', function(done) {
			...
			cicod.getStakingInfo(function(err, result) {
				should.exist(err);
				cicod._tryAllClients.callCount.should.equal(1);
