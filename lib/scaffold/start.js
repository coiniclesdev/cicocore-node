...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for cicod is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and cicocore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed cicocore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './cicocore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in cicocore-node services directory
    ...
    if (servicePackage.cicocoreNode) {
      serviceModule = service.name + '/' + servicePackage.cicocoreNode;
    }
